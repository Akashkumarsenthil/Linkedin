...
These endpoints require a valid JWT so that raw infrastructure data is not exposed
to unauthenticated callers.  They are read-only and return no sensitive user data.
...
async def kafka_stats(_: TokenPayload = Depends(get_current_user)):
    """
...
async def perf_cache_stats(_: TokenPayload = Depends(get_current_user)):
    """
...
def mysql_stats(db: Session = Depends(get_db), _: TokenPayload = Depends(get_current_user)):
    """
...
async def bench_results(_: TokenPayload = Depends(get_current_user)):
    """
...
async def event_trend(days: int = 14, _: TokenPayload = Depends(get_current_user)):
    """
